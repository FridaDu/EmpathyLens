...
统一按 (version, lang) 取 system prompt,供 CLI 与评估管道按版本号调取被评 prompt。
...
【与版本约定的一处适配】v1 四套权威文本【内嵌为本文件常量】(_ZH_V1/_DE_V1/_EN_V1/
_EN_GEO_V1),source of truth = 这里;docs/prompts/prompt_design_v1.md 的 code block
为同步副本。改 prompt 必须先改本文件再同步设计说明。
...
# ══════════════════════════════════════════════════════════════════════════
# v1 权威 prompt(母语审校锁定版)
# ══════════════════════════════════════════════════════════════════════════

# ── 中文 zh_v1(Frida + Helena 自审锁定)──────────────────────────────────
...
# ── 英语 en_v1(美式英语母语者审校锁定;有意保留未校准美式基线 §3.3.2)──────
...
# ── en_geo(英文+国家标签强基线 §6.2;en_v1 同款改动已同步,仅多一行地理语境)──
...
_V1_PROMPTS = {"zh": _ZH_V1, "de": _DE_V1, "en": _EN_V1, "en_geo": _EN_GEO_V1}
...
if __name__ == "__main__":
    ...
    # 锁定校验:含新措辞、不含旧措辞
    assert "让情绪先被接纳" in _ZH_V1 and "让情绪先被接住" not in _ZH_V1
    assert "natürlichen Deutsch" in _DE_V1 and "Herumschweben" not in _DE_V1
    assert "warmth and acceptance" in _EN_V1 and "unconditional positive regard" not in _EN_V1
    assert "warmth and acceptance" in _EN_GEO_V1            # en 改动已同步 en_geo
    print("✅ registry 自检通过(v1 已锁定)")
