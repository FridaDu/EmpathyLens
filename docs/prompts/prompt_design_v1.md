...
> Authors: Frida Du · Helena Cai · Week 7 · zh 主导 Frida Du(L1 中文)/ de 主导 Frida(非母语,待审校)/ en 主导 Helena Cai
> 落地 framework_CN_v2 §3.1 / §3.2 / §3.3 / §3.0 / §5.4 / §5.5 / §6.2 / §6.4 + evaluation_dimensions_v1.1
> 配套:`prompt_versioning_convention.md`(版本约定)、`week7_sanity_check.md`(抽样+测试记录表)
> 本文件含三套 prompt 的**权威文本(code block)+ 逐条设计依据 + 七维对齐表 + 局限**。下方 code block 的四套 prompt 已内嵌为src/prompts/registry.py 的常量(_ZH_V1 / _DE_V1_DRAFT / _EN_V1 / _EN_GEO_V1),代码为权威。

---

# 0. 总览与设计总原则

Week 7 产出 = Week 8 评估管道要打分的"被评对象"。zh / de 用 Week 6 七维框架 + §3.0 四轴 + §5.4 意图簇 + §5.5 M 码反向规约,深化为 **calibrated v1**;en **故意保持未校准基线**(§3.3.2),作 Equitability 对照。三套统一用 Week 4 的五块结构:角色定义 / 行为规范 / 边界约束 / 输出格式 / 异常处理。
...
3. **每种语言都要母语者把关(§6.4)**:framework 引 HoMemeTown 韩语版"语法对但语用不对"的教训,要求每语文案分别请母语者审。落到三套 prompt:zh 已由 Frida(母语)覆盖;**de 由德语母语者 Jens 全权审校(见 §2)**;en 由 Helena 协调美式英语母语者轻审(§3.3.2 警告"英语≠美国白人中产英语")。团队原始数据集文案本就全部母语者撰写,prompt 审校沿用同一批母语者。
...
角色定义重定向到"不孤单/被理解/不丢面子"(§3.1.1 规范一 + §3.1.4 面子 → Preserve-Face 意图),对应 **D1/D5**。行为规范:在场优先(§3.0 A 轴 + §3.1.1,**D4/D7**)、让情绪先被接住(§3.0 B 轴 + §5.4 Cathart,**D5/D4**)、关系性归因(§3.1.3 + 不犯 M1/M4,**D4**)、维护体面(§3.1.4 Preserve-Face,**D5/D2**)、含蓄留白(§3.0 D 轴 + §3.1.2,**D4/D6**)、长期视角非美式正面化(§3.0 C 轴 + §2.4,**D4/D2**)。边界约束反向规约 §5.5:个体主义覆盖=M1、对不同关系同姿态=M4、安慰变改变说教=M5、未接住先解决=M6、临床诊断腔=M8、翻译腔=M2/M3 → 服务 **D4/D5/D6/D3**。输出格式锁 §3.1.2 短回应多轮 → **D6**(+ D7 留推进空间)。异常处理:对抗按 §4.2/§5.5 M7 tension-naming → **D7**;危机按 evaluation_dimensions_v1.1 §3 D3 锚点 + §6.4 资源(010-82951332)→ **D3**;共同底线 → **D1/D2/D3**。
...
| D5 意图契合 | 让情绪先被接住(Cathart) + 维护体面(Preserve-Face) + 边界 M5 | §5.4 |
...
```text
Antworte immer auf natürlichem Deutsch, unabhängig davon, in welcher Sprache die Person schreibt.
...
- **⚠️ 母语审校硬前置(最重要),由 Jens 全权负责**:本套德语由非母语者(Frida)依 §3.2 撰写。prompt 自身若语体/虚拟式/搭配不自然,会**直接污染 D6**(分不清是模型德语差还是 prompt 德语差)。**锁定前必须由德语母语者 Jens 过一遍自然度,只改语言形式、不改规范结构**;Jens 审校通过前记为 `de_v1-draft`,通过后才升 `de_v1`。这正是 §6.4 引 HoMemeTown 教训要防的。
...
en+geo = **en 未校准基线 + 唯一一条地理语境线**,**不注入任何四轴/意图/M码校准**——注入了它就变成我们的 calibrated 条件,不再是 CuLEmo 基线,对比就失效。响应语言为英文。安全资源按国别本地化(§6.4,属三套共享的安全基线,不算支持风格的文化校准)。

## 4.3 prompt 模板(参数化,权威副本)
...
> 与纯 en 基线的唯一差别 = 那条 "lives in {COUNTRY}…" 地理语境线(+ 本地化危机资源)。其余文本与 en_v1 完全一致,确保"标签 vs 无标签"是干净的单一自变量。
...
**四条评估条件一览**:`zh_v1`(calibrated)、`de_v1`(calibrated,待 Jens 审)、`en_v1`(uncalibrated baseline)、`en+geo`(country-label strong baseline)。
...
**母语审校状态(§6.4)**:zh ✅(Frida 母语)/ de → **Jens 全权审校**(通过前记 `de_v1-draft`)/ en → Helena 协调美式英语母语者轻审。

---

*v1 设计说明合并版。三套将在 Week 8 首批响应试评后据裁判间一致性 + sanity 结果迭代为 v2。de 在母语审校完成前记 `de_v1-draft`。*
